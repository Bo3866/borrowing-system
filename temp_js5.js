...
function isAlcoholEnabled() {
                                ...
                                function validateAlcoholForm() {
                                    ...
                                    if (!allChecked) { alert('隢??暸銝衣Ⅱ隤摰?蝎暸ㄡ?暑???乓????蝭???); return false; }
                                    if (!coordinator) { alert('隢‵撖怒?蝎暸ㄡ?暑???乓?瘣餃?鞎痊鈭箝?); return false; }
                                    if (!president) { alert('隢‵撖怒?蝎暸ㄡ?暑???乓?蝷暸??); return false; }
                                    return true;
                                }
                                ...
                                function is30DaysRequired() {
                                    ...
                                    return (participantCount === '100~200鈭? || participantCount === '200鈭箔誑銝?) || hasAlcohol || hasFire || hasSales;
                                }
                                ...
                                function validateStartDate() {
                                    ...
                                    if (req30) {
                                        ...
                                        if (selectedDate < min30Date) { errorMsg = '瘜冽?嚗?潭?暑???怎畾扯釭嚗?蝎整??怒鞎拇?頞?100鈭綽?嚗?? 30 憭拐??隢?\n蝟餌絞撌脫?蝛箔??????隢??圈?撠 ' + formatDate(min30Date) + ' ???; }
                                    }
                                    if (!errorMsg && reqFlag) {
                                        const minFlagDateStr = getMinFlagDate(); const minFlagDate = new Date(minFlagDateStr); minFlagDate.setHours(0,0,0,0);
                                        if (selectedDate < minFlagDate) { errorMsg = '瘜冽?嚗?蝡?撟?雿輻?交??芾?豢? 7 ?極雿予銋?????喳???' + minFlagDateStr + '嚗?\n蝟餌絞撌脩?冽?蝛箔????暑??憪??隢??圈??; }
                                    }
