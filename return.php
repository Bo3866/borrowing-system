...
<body class="bg-slate-50 text-slate-800">
    <nav class="navbar bg-indigo-900 text-white p-4 flex justify-between items-center shadow-md">
        <div class="navbar-brand"><h1 class="text-xl font-bold">📚 校園資源租借系統</h1></div>
        <div class="navbar-menu flex gap-3">
            <button class="nav-btn bg-indigo-700 px-4 py-2 rounded-lg text-sm hover:bg-indigo-600 transition" onclick="location.href='index.php'">回首頁</button>
            <button class="nav-btn bg-indigo-700 px-4 py-2 rounded-lg text-sm hover:bg-indigo-600 transition" onclick="location.href='report_maintenance.php'">報修</button>
            <button class="nav-btn bg-indigo-800 px-4 py-2 rounded-lg text-sm" type="button" disabled><?php echo htmlspecialchars($_SESSION['full_name'] ?? $_SESSION['user_id'], ENT_QUOTES, 'UTF-8'); ?></button>
            <button class="nav-btn bg-rose-700 px-4 py-2 rounded-lg text-sm hover:bg-rose-600 transition" onclick="location.href='logout.php'">登出</button>
        </div>
    </nav>
                            ...
                            <tbody>
                                ...
                                <?php } else { ?>
                                    <?php foreach ($rows as $row) { ?>
                                        ...
                                        ?>
                                        <!-- 移除表格 tr 的 cursor-pointer，使其點擊整行沒有反應 -->
                                        <tr class="accordion-trigger hover:bg-slate-50/50 transition"
                                            ...
                                            <td class="left-trigger-zone text-center text-indigo-600 font-bold transition" 
                                                ...
                                                onclick="toggleAccordion(this.closest('tr'), <?php echo (int)$row['reservation_id']; ?>)">
                                            ...
                                            <td style="text-align: center;">
                                                <?php if ($approvalStatus === 'pending') { ?>
                                                    <a href="edit_application.php?reservation_id=<?php echo (int)$row['reservation_id']; ?>" class="bg-indigo-600 text-white px-2.5 py-1 rounded text-xs inline-block hover:bg-indigo-700 transition">修改申請</a>
                                                <?php } else { ?>
                                                    -
                                                <?php } ?>
                                                <div style="margin-top:6px;">
                                                    <!-- 查看詳情按鈕：直接傳入該列元素，觸發開窗 -->
                                                    <button class="detail-button" onclick="openDrawer(this.closest('tr'))">查看詳情</button>
                                                </div>
                                            </td>
                                            <td>
                                                <?php if ($isReturned) { ?>
                                                ...
                                                <?php } elseif ($approvalStatus === 'approved' && $isCheckedIn) { ?>
                                                    <form method="post" class="inline-block m-0">
                                                        ...
                                                        <button type="submit" class="bg-slate-800 text-white px-3 py-1 rounded text-xs hover:bg-slate-700 transition" onclick="return confirm('確認此申請已歸還或離場？')">確認歸還／離場</button>
                                                    </form>
                                                <?php } elseif ($approvalStatus === 'approved' && !$isCheckedIn) { ?>
