...
// 先計算借用時間（用於後續驗證）
        ...
        if (
            ...
            if (
                ...
                // 實際領取/進入日期：只能是活動開始日 -1 天，或活動開始日。
                // 只用日期 +1/-1 判斷，不改其他時間規則。
                if ($borrowError === '' && $formData['borrow_start_date'] !== '') {
                    $borrowStartDateTime = DateTime::createFromFormat('Y-m-d', $formData['borrow_start_date']);
                    if (!$borrowStartDateTime) {
                        $borrowError = '借用開始日期格式有誤，請重新選擇。';
                    } else {
                ...
                // 實際歸還/離開日期：只能是活動結束日，或活動結束日 +1 天。
                // 只用日期 +1/-1 判斷，不改其他時間規則。
                if ($borrowError === '' && $formData['borrow_end_date'] !== '') {
                    $borrowEndDateTime = DateTime::createFromFormat('Y-m-d', $formData['borrow_end_date']);
                    if (!$borrowEndDateTime) {
                        $borrowError = '借用結束日期格式有誤，請重新選擇。';
                    } else {
                        ...
                        if (!in_array($formData['actual_return_date'], $allowedReturnDates, true)) {
...
function goToStep(stepNo) {
    ...
    if (stepNo > 1 && currentStep === 1) {
        ...
        if (returnDateEl && returnDateEl.value && borrowEndDateEl && borrowEndDateEl.value) {
            ...
            if (actualReturnDate < borrowEndDate || actualReturnDate > maxReturnDate) {
                alert("實際歸還/離開日期只能選活動結束日或活動結束日後一天！");
                returnDateEl.focus();
                return;
            }
